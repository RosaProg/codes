...
function slideWidget(name){
	if (name=='_lite_widget'){
		jQuery('#simple_widget:visible').slideToggle(200);
		jQuery('#_lite_widget').slideToggle(200);
	} else if (name=='simple_widget') {
		jQuery('#_lite_widget:visible').slideToggle(200);
...
function getTotalAmounts(recalc_spot_donation){
	...
	if(jQuery('#_credit_amount').length > 0) {
		credit = parseFloat(jQuery('#_credit_amount').html().replace(',', '')) || 0;
	}
	else
	{
		credit = 0;
	...
	if(recalc_spot_donation && owe_amount > 0){
		jQuery('#_donation').attr("value",formatAsMoney(owe_amount/20));
		jQuery('#_donation_hidden').attr("value",formatAsMoney(owe_amount/20));
	}
	else if (recalc_spot_donation)
	{
		jQuery('#_donation').attr("value","0.00");
		jQuery('#_donation_hidden').attr("value","0.00");
	}

	spot_us_support = parseFloat(jQuery('#_donation').val()) || 0;
	...
	jQuery('#_total_amount').html('$'+formatAsMoney(total_amount));
...
jQuery(document).ajaxComplete(function(options, r) {
	var notice;
	var dismiss = "<span class=\"dismiss\"><a href=\"\"><img src=\"http://www./images/close_square.png\" alt=\"Dismiss\" /></span>";
