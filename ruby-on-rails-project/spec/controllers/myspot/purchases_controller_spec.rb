...
describe Myspot::PurchasesController do
  ...
  describe "on GET to new when the user has unpaid donations" do
    ...
    it "should allow only  donations" do
      donations = stub('named_scope', :unpaid => [])
      @user.stub!(:donations).and_return(donations)
      @user.stub!(:unpaid__donation).and_return(Factory(:_donation))
  ...
  describe "PayPal response" do
    before do
      @paypal_params =  {"protection_eligibility"=>"Eligible", "tax"=>"0.00",
        ...
        "txn_id"=>"28C98632UU123291R", "item_name2"=>"Support ",
    ...
    describe "when the payment has been completed" do
      ...
      before do
        ...
        @donations = [@donation]
        @_donation = mock_model(Donation, :amount => 40.00, :unpaid? => true, :user => @user)
        Donation.stub!(:find_from_paypal).and_return(@_donation)
      ...
      it "finds  donation" do
        Donation.should_receive(:find_from_paypal).with(@paypal_params).and_return(@_donation)
        do_post
      end
      ...
      it "creates a new purchase" do
        ...
        @purchase.should_receive(:_donation=).with(@_donation)
