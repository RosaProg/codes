...
describe User do
  ...
  table_has_columns(User, :boolean,  "notify__news")
  ...
  describe "generate_csv" do
    it "should return a comma-separated list" do
      user = Factory(:citizen, :type => "Citizen", :email => 'happy@happy.com',
                     ...
                     :notify_stories => "true", :notify__news => "true",
  ...
  describe "with  donations" do
    before do
      @user = Factory(:citizen)
      @_donation_one = Factory(:_donation, :purchase => Factory(:purchase), :user => @user)
      @_donation_two = Factory(:_donation, :purchase => nil, :user => @user)
    end

    it "should return the current unpaid  donation" do
      @user.current__donation.should == @_donation_two
    end

    it "should have  donations" do
      @user.has__donation?.should be_true
    end

    it "should have an unpaid  donation" do
      @user.unpaid__donation.should == @_donation_two
    end

    it "should return the sum of paid  donations" do
      @user.paid__donations_sum.should == @_donation_one.amount
    end
  end

  describe "without any  donations" do
    before do
      @user = Factory(:citizen)
    end

    it "should return a new record for current__donation" do
      @user.current__donation.should be_a_new_record
    end

    it "should return a Donation for current__donation" do
      @user.current__donation.should be_a_kind_of(Donation)
    end

    it "should not have  donations" do
      @user.has__donation?.should be_false
    end

    it "should return 0 for the sum of the paid  donations" do
      @user.paid__donations_sum.should == 0
    end
  end
end
