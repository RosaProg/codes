...
describe 'purchases/new' do
  before do
    ...
    @donations = @pitches.collect {|pitch| Factory(:donation, :user => @user,
                                                              :pitch => pitch,
                                                              :purchase => nil) }
    @_donation = Factory(:_donation, :user => @user, :amount => 1, :purchase => nil)
  ...
  describe "PayPal" do
    ...
    it "includes the  donation" do
      do_render
      template.should have_tag("input[name='item_name_#{@donations.size + 1}'][value=?]", "Support ")
      template.should have_tag("input[name='amount_#{@donations.size + 1}'][value=?]", @_donation.amount)
    end
