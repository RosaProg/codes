...
describe 'myspot/donation_amounts/edit' do
  before do
    ...
    @donations = @pitches.collect {|pitch| Factory(:donation, :user => @user,
                                                              :pitch => pitch) }
    @_donation = Factory(:_donation)
    @group = Factory(:group)
    template.stub!(:unpaid_donations).and_return(@donations)
    template.stub!(:_donation).and_return(@_donation)
  end
  ...
  it "should display the  donation field" do
    do_render
    template.should have_tag('input[type="text"][name=?]', "_donation_amount")
