namespace :credits do  
  ...
  task :correct => :environment do
    ...
    # delete all donations with amount-0
    Donation.delete_all('amount<=0')
    
    _donations = Donation.find(:all, :conditions=>"credit_id is not null")
    puts %|------------------------------------------------------------------------------------------------|
    puts %|   Starting to process the  donations... |
    puts %|------------------------------------------------------------------------------------------------|
    user_ids = []
    processed_donations = 0
    non_processed_donations = 0
    _donations.each do |_donation|
      puts %|          Processing _donation #{_donation.id}...|
      user_ids << _donation.user_id
      old_credit = Credit.find(:first, :conditions=>"user_id=#{_donation.user_id} and amount=#{_donation.amount} and id not in (#{corrected_credits.join(',')})", :order=>"created_at desc")
      active_credit = _donation.credit
      if old_credit && active_credit
        active_credit.amount = old_credit.amount
        active_credit.cca_id = old_credit.cca_id
        active_credit.save
        ...
        puts %|             --- Creating new credit for _donation #{_donation.id}...|
        active_credit = Credit.create(:user_id => _donation.user_id, :description => "Donated to ",
                        :amount => _donation.amount)
        _donation.credit.destroy if _donation.credit
        _donation.credit_id = active_credit.id
        _donation.save
        processed_donations += 1
      end
    ...
    puts %|------------------------------------------------------------------------------------------------|
    puts %| Number of  donations: #{_donations.length} |
    puts %| Number of users: #{user_ids.uniq.length} |
    puts %| Number of processed  donations: #{processed_donations}|
    puts %| Number of unprocessed credits for  donations: #{Donation.find(:all, :conditions=>"credit_id is not null and credits.amount<0", :include=>:credit).length} | 
    puts %|------------------------------------------------------------------------------------------------|
  ...
  task :transfer => :environment do
    ...
    credits = Credit.find(:all,
      ...
      :joins => "LEFT JOIN donations ON donations.credit_id = credits.id LEFT JOIN _donations ON _donations.credit_id = credits.id", 
      :conditions => "donations.id is null and _donations.id is null", 
    ...
    credits.each do |c|
      user_credits = Credit.find(:all,
          :select => "credits.*", 
          :joins => "LEFT JOIN donations ON donations.credit_id = credits.id LEFT JOIN _donations ON _donations.credit_id = credits.id", 
          :conditions => "donations.id is null and _donations.id is null and credits.user_id=#{c.user_id}")

      user_credits.each do |credit|
        puts %|       Transferring funds for #{credit.user.full_name} with the credit amount #{credit.amount} |
        _donation = Donation.create({ :amount => credit.amount, :user_id => credit.user_id, :credit_id => credit.id  })
      end
    end
