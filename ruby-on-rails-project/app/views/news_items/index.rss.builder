...
xml.rss :version => "2.0" do
  xml.channel do
    xml.title ": #{@filter.titleize} Stories From #{get_inside_network_text(@network).titleize}"
    xml.link request.url
    xml.description "These are the #{@filter.downcase} stories from  #{get_inside_network_text(@network)}."
    xml.language "en-us"
