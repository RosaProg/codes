class Myspot::PurchasesController < ApplicationController
  ...
  def create
    ...
    _amount = params[:_donation_amount]
    ...
    # create the  donation
    _donation = Donation.create(:user_id => current_user.id, :amount => _amount)
    
    @purchase.donations       = [d]
    @purchase._donation = _donation
    ...
    begin
      if @purchase.save
        ...
        redirect_to cookies[:_lite] ? "/lite/#{cookies[:_lite]}" : redirect_url
      else
        unless cookies[:_lite]
          render :action => 'new'
        else
  ...
  def paypal_return
    ...
    redirect_to cookies[:_lite] ? "/lite/#{cookies[:_lite]}" : myspot_donations_path
  end
  ...
  def paypal_ipn
    ...
    _donation_amount = params[:mc_gross_2]
    ...
    # create the  donation
    _donation = Donation.create(:user_id => user_id, :amount => _donation_amount)
    
    unless _donation.errors.empty?
      logger.info('Donation errors:')
      _donation.errors.each do |key,e|
        logger.info("Error: " + e)
      end
    end
  ...
  def unpaid_donations_required
    # redirect_to myspot_donations_path if current_user.donations.unpaid.empty? && (current_user.unpaid__donation.nil? || current_user.unpaid__donation.amount <= 0)
  end
