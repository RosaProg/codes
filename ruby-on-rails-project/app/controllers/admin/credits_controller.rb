class Admin::CreditsController < ApplicationController
  ...
  def transfer
    ...
    credits = Credit.find(:all,
        :select => "credits.*", 
        :joins => "LEFT JOIN donations ON donations.credit_id = credits.id LEFT JOIN _donations ON _donations.credit_id = credits.id", 
        :conditions => "donations.id is null and _donations.id is null and credits.user_id=#{params[:user_id]}")
        
    credits.each do |credit|
      _donation = Donation.create({ :amount => credit.amount, :user_id => user.id, :credit_id => credit.id  })
    end
