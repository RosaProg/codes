class PitchesController < ApplicationController
  ...
  def apply_credits
    ...
    credits.each do |credit|
      amount = credit.amount * (1-Donation::_TITHE)
      
      # slice off the  donation
      _donation_credit = Credit.create(:user_id => credit.user_id, :description => "#{credit.description} (Sliced off from #{credit.id} which had the amount #{credit.amount})",
      ...
      # create the  donation
      _donation = Donation.create(:user_id => current_user.id, :credit_id => _donation_credit.id, :amount => _donation_credit.amount)
    end
  ...
  def can_edit?
    ...
    if not pitch.editable_by?(current_user)
        ...
        #if pitch.donated_to?
        #  access_denied( \
        #    :flash => "You cannot edit a pitch that has donations.  For minor changes, contact info@",
        #    :redirect => pitch_url(pitch))
        #else
        #  access_denied( \
        #    :flash => "You cannot edit this pitch.  For minor changes, contact info@",
        #    :redirect => pitch_url(pitch))
        #end
