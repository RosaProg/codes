class UsersController < ApplicationController
  ...
  def create
    ...
    return_path = params[:return_to] if params[:return_to] && params[:_lite]
    return_path = URI.decode(cookies[:return_to]) if cookies[:return_to]
    return_path = session[:return_to] if session[:return_to]
    if verify_recaptcha(:model => @user, :message => "You did not specify a proper captcha.") &&@user.save
      unless @user.organization?
        ...
        flash_and_redirect(:success, 'Welcome to !', return_path)
      else
        if award_redeem_credits_if_any
          redirect_to "/stories/unfunded"
          return
  ...
  def activate
    ...
    if current_user
      ...
      handle_first_donation_for_non_logged_in_user
      handle_first_pledge_for_non_logged_in_user
      flash[:success] = "You have successfully activated your account - welcome to !"
