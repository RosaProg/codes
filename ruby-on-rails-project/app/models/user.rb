...
#  notify__news        :boolean(1)      not null
...
class User < ActiveRecord::Base
  ...
  attr_accessible :about_you, :address1, :address2, :city, :country,
    ...
    :location, :notify_blog_posts, :notify_comments, :notify_pitches, :notify__news, :notify_stories,
...
########### end facebook ##############
  ...
  def total_available_credits
    self.credits - self.all_donations.find(:all, :conditions=>"credit_id is not null", :include=>:credit).map(&:credit) - self._donations.find(:all, :conditions=>"credit_id is not null", :include=>:credit).map(&:credit)
  end
  ...
  def current_balance
    unpaid_donations_sum + current__donation.amount #unpaid_donations_sum - allocated_credits + current__donation.amount
  end
  ...
  def self.generate_csv
    FasterCSV.generate do |csv|
      ...
      csv << ["type", "email", "first_name", "last_name", "network", "notify_blog_posts", "notify_comments",
              ...
              "notify_stories", "notify__news", "fact_check_interest"]
      ...
      User.all.each do |user|
        csv << [user.type, user.email, user.first_name, user.last_name,
                ...
                user.notify_stories, user.notify__news, user.fact_check_interest]
      end
    end
  end
  ...
  def unpaid__donation
    _donations.unpaid.first
  end

  def current__donation
    unpaid__donation || _donations.build
  end
  ...
  def paid__donations_sum
    has__donation? ? _donations.paid.map(&:amount).sum : 0
  end

  def last_paid__donation
    _donations.paid.last
  end
