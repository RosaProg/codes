...
class Pitch < NewsItem
  ...
  def suggested_funding
    return funding_needed if funding_needed>0 && funding_needed<Donation::DEFAULT_AMOUNT
  ...
  def create_peer_editor_assignment
    Assignment.create(:pitch_id => self.id, :user_id => self.user.id, :title =>"Apply to be Peer Review Editor", :is_factchecker_assignment => true,
                      ...
                      " At any time the Peer-Review editor can also report suspicious activities to ."].join(""))
  end
