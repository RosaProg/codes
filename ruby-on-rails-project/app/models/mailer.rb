class Mailer < ActionMailer::Base
  ...
  def activation_email(user)
    ...
    from       get_email(:info)
    subject    %(Welcome to  – Please verify your email address")
    body :user => user
  end
  ...
  def citizen_signup_notification(user)
    recipients user.email
    from       get_email(:info)
    subject    %(Welcome to  – "Community Funded Reporting")
    body :user => user
  end
  ...
  def reporter_signup_notification(user)
    recipients user.email
    from       get_email(:info)
    subject    "Welcome to  – Reporting on Communities"
    body :user => user
  end
  ...
  def sponsor_interest_notification(user)
    ...
    from       get_email(:info)
    subject    " - Someone has signed up to a Sponsor"
    body :user => user
  end
  ...
  def organization_signup_notification(user)
    recipients user.email
    from       get_email(:info)
    subject    ": Important Information on Joining"
    body :user => user
  end
  ...
  def news_org_signup_request(user)
    ...
    from       get_email(:info)
    subject    ": News Org Requesting to Join"
    body        :user => user
  end
  ...
  def pitch_created_notification(pitch)
    recipients get_email(:pitch)
    from       get_email(:info)
    subject    ": A pitch needs approval!"
    body       :pitch => pitch
  end
  ...
  def pitch_edited_notification(pitch)
    ...
    subject    ": A pitch has been edited!"
    body       :pitch => pitch
  end
  ...
  def pitch_accepted_notification(pitch,to,email,subscriber=nil)
    recipients  email
    from       get_email(:info)
    subject    ": Success!! Your Story is Funded!"
  ...
  def blog_posted_notification(post,to,email,subscriber=nil)
    recipients  email
    from       get_email(:info)
    subject    " Blog Update: '#{post.pitch.headline}'"
  ...
  def comment_notification(comment, user)
    recipients  user.email
    from       get_email(:info)
    subject    " Comment: '#{comment.commentable.headline}'"
  ...
  def story_to_editor_notification(story,to,email)
    recipients email
    from       get_email(:info)
    subject    " Story Review: '#{story.headline}'"
  ...
  def story_rejected_notification(story)
    ...
    bcc        get_email(:admin)
    from       get_email(:info)
    subject    " Story Requires Further Edits: '#{story.headline}'"
    body       :story => story
  end
  ...
  def admin_reporting_team_notification(pitch)
    ...
    from       get_email(:info)
    subject    ": Someone wants to join a pitch's reporting team!"
    body       :pitch => pitch
  end
  ...
  def reporter_reporting_team_notification(pitch)
    recipients pitch.user.email
    from       get_email(:info)
    subject    ": Someone wants to join your reporting team!"
    body       :pitch => pitch
  end
  ...
  def create_blog_post_notification(pitch)
    recipients pitch.user.email
    from       get_email(:info)
    subject    ": Create a blog post, build community and interest around the post!"
    body       :pitch => pitch
  end
  ...
  def unpaid_donations(user)
    recipients user.email
    from       get_email(:info)
    subject    ": You have unpaid donations!"
    body       :user => user
  end
  ...
  def approved_reporting_team_notification(pitch, user)
    recipients user.email
    from       get_email(:info)
    subject    ": Welcome to the reporting team!"
  ...
  def assignment_application_notification(mail)
    ...
    bcc        get_email(:admin)
    from       get_email(:info)
    subject    ": Assignment assignment application: " + mail[:assignment].pitch.slug?
    body       mail
  end
  ...
  def assignment_application_accepted_notification(assignment, user)
    recipients user.email
    from       get_email(:info)
    subject    ": Assignment application accepted!"
  ...
  def assignment_application_rejected_notification(assignment, user)
    recipients user.email
    from       get_email(:info)
    subject    ": Assignment application rejected!"
    body       :assignment => assignment, :user => user
  end
  ...
  def story_ready_notification(story)
    recipients get_email(:admin)
    from       get_email(:info)
    subject    ": Story ready for publishing"
    body       :story => story
  end
  ...
  def confirm_subscription(subscriber)
    ...
    from        get_email(:info)
    subject     ": Subscription to '#{subscriber.pitch.headline}' confirmation required"
  ...
  def thank_you_for_your_pitch(user, pitch)
    recipients  user.email
    from        get_email(:info)
    subject     "Thanks for creating a pitch on !"
    body        :pitch => pitch, :user => user
  end
  ...
  def reporting(start_date, end_date, interval, has_donations=true)
    ...
    from        get_email(:info)
    subject     ": #{interval.capitalize} Report for donations between #{start_date.strftime("%m/%d/%y")}-#{end_date.strftime("%m/%d/%y")}"
