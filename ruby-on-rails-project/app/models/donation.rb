...
class Donation < ActiveRecord::Base
    ...
    if (intervals.include?(interval) && params.length == 1) || 
      ...
      donations.each do |d|
      	...
      	if d.purchase
        	...
        	txt << (d.purchase._donation ? d.purchase._donation.amount : 0)
  ...
  def update_facebook
    #unless Rails.env.development?
      if user.notify_facebook_wall
        ...
        user.save_async_post(" Donation: I have just donated to this pitch.", description, pitch.short_url, pitch.featured_image.url, pitch.headline) if user
      end
    #end
  end
  ...
  def self.find_all_from_paypal(paypal_params)
    strip__donations(paypal_params)
  ...
  def self.strip__donations(paypal_params)
    if _keys = paypal_params.detect{|k,v| v =~ /support spot\.us/i}
      paypal_params.delete(_keys.first.gsub(/name/, 'number'))
      paypal_params.delete(_keys.first)
    end
  end
