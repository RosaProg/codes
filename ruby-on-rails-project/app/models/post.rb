...
class Post < ActiveRecord::Base
  ...
  def update_facebook
    ...
    if user.notify_facebook_wall
      ...
      [self.user, User.info_account?].compact.uniq.each do |u|
        u.save_async_post(" Blog Post", description, self.short_url, self.blog_image.url, self.title) if u && u.facebook_user?
      end
    end
    #end
  end
  ...
  def blog_posted_notification
    ...
    #email admins
    conditions = "email not in (#{emails.join(',')})" if emails && !emails.empty?
    Admin.find(:all,:conditions=>"email!='kara@' and email not in (#{emails.join(',')})").each do |admin|
