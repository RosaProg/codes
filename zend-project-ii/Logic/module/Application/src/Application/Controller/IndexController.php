...
class IndexController extends AbstractActionController
{
 public $dbAdapter;
    public function indexAction()
    {
        ...
        $camara=$this->dbAdapter->query("select * from camara where estadoregistro='ASC'",Adapter::QUERY_MODE_EXECUTE);
		...
		$del_camara=$this->dbAdapter->query("select * from camara where estadoregistro='DEL'",Adapter::QUERY_MODE_EXECUTE);
	  ...
	  if(@$_POST['undelete_camara']){
	  $this->undeleteAction(@$_POST['undelete_text']);
	  }
...
$result= $this->dbAdapter->query('UPDATE `camara` SET tipo= "'.@$_POST['tipo'].'" , codigo = "'.@$_POST['codigo'].'", descripcion = "'.@$_POST['descripcion'].'",posicion="'.@$_POST['posicion'].'",estado="'.@$_POST['estado'].'",ip="'.@$_POST['ip'].'",marca="'.@$_POST['marca'].'",modelo="'.@$_POST['modelo'].'",altausuario="'.@$_POST['altausuario'].'",altafecha="'.@$_POST['altafecha'].'",modiusuario="'.@$_POST['modiusuario'].'",modifecha="'.@$_POST['modifecha'].'",bajafecha="'.@$_POST['bajafecha'].'",bajausuario="'.@$_POST['bajausuario'].'",estadoregistro="ASC" WHERE id='.$id, Adapter::QUERY_MODE_EXECUTE);	
	...
	public function deleteAction($id){
		$sql = new Sql($this->dbAdapter);
		$result= $this->dbAdapter->query('UPDATE `camara` SET estadoregistro="DEL"   WHERE id='.$id, Adapter::QUERY_MODE_EXECUTE);	
	...
	if($id!=''){
	$sql = new Sql($this->dbAdapter);
		$result= $this->dbAdapter->query('UPDATE `camara` SET estadoregistro="ASC"   WHERE id='.$id, Adapter::QUERY_MODE_EXECUTE);	
        /*$deleteString = 'DELETE FROM camara WHERE id ='.$id.'';
        $result= $this->dbAdapter->query($deleteString, Adapter::QUERY_MODE_EXECUTE);*/
		return $this->redirect()->toRoute('home', array(
    'save' => 'su'));
